...
🛠️ Troubleshooting
Sensor not found? Ensure the White Wire is connected between Teensy Pin 2 and BNO RST. This pulses the sensor on startup to prevent I2C hang-ups.

Cursor off-center? Ensure the NATIVE_X/Y and SCALE variables in the code match your Windows Display settings exactly.

Axes Inverted? If Left is Right, swap the mapping values in the code: map(..., RES_X, 0) ➔ map(..., 0, RES_X).
